...
type Tab = "overview" | "posts" | "books" | "videos" | "subscribers" | "slides";
...
export function AdminDashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      (rp.data || []).forEach((x) =>
        activity.push({ type: "Post", title: x.title, date: x.created_at }),
      );
      (rb.data || []).forEach((x) =>
        activity.push({ type: "Book", title: x.title, date: x.created_at }),
      );
      (rv.data || []).forEach((x) =>
        activity.push({ type: "Video", title: x.title, date: x.created_at }),
      );
      activity.sort(
        ...
        <div className="p-5 border-b border-amber-400/10">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-slate-500 text-xs truncate max-w-[110px]">
                {user?.email}
              </p>
            </div>
          </div>
              ...
              <div className="glass-card rounded-xl p-5">
                ...
                {recentActivity.length === 0 ? (
                  <p className="text-slate-500 text-sm py-4 text-center">
                    No content yet. Start adding posts, books, or videos!
                  </p>
                ) : (
